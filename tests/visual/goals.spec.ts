...
test('goal stats open beside search and adapt their date range', async ({ page }) => {
  ...
  await expect(dialog.getByText('Each bar is the number of goals completed that day')).toHaveCount(0)
  await expect(dialog.locator('.line-y-axis')).toBeVisible()
  await expect(dialog.locator('.chart-card').filter({ hasText: 'Completion activity' }).locator('.chart-y-axis')).toBeVisible()
  ...
  await expect(deadlineChart.locator('.chart-x-axis')).toContainText('Today')
  await expect(deadlineChart.locator('.chart-x-axis')).toContainText('1 day')
  await expect(deadlineChart.locator('.chart-x-axis')).toContainText('7 days')
