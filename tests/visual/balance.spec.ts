...
test('IMAX mode maximizes Today and restores its surrounding panels', async ({ page }, testInfo) => {
  ...
  await expect(primaryPane.locator('.page-header')).toBeHidden()
  ...
  if (testInfo.project.name === 'desktop') {
    ...
    await page.locator('.imax-exit-control').getByRole('button', { name: 'Exit IMAX mode' }).click()
  ...
  await expect(page.locator('.imax-exit-control').getByRole('button', { name: 'Exit IMAX mode' })).toHaveAttribute('aria-pressed', 'true')
  await page.keyboard.press('Alt+i')
  await expect(imaxButton).toHaveAttribute('aria-pressed', 'false')
