...
test('Escape leaves task editing; held W/S accelerates linearly and stops immediately', async ({ page }) => {
  ...
  // At 600 px/s with a linear 40% ramp over 800 ms, the first and second
  // 400 ms intervals cover 264 px and 312 px respectively.
  expect(first - initial).toBeGreaterThan(255)
  expect(first - initial).toBeLessThan(275)
  expect(second - first).toBeGreaterThan(303)
  expect(second - first).toBeLessThan(323)
  await page.clock.runFor(400)
  const third = await scrollTop()
  // The ramp is complete, so the chosen 600 px/s runs at 140%: 840 px/s.
  expect(third - second).toBeGreaterThan(326)
  expect(third - second).toBeLessThan(346)
  ...
  expect(third - await scrollTop()).toBeGreaterThan(330)
  await editor.focus()
...
test('admin settings show defaults, persist scroll tuning and reset it; studio moved out of Settings', async ({ page }) => {
  ...
  await expect(acceleration).toHaveValue('800')
  await expect(increase).toHaveValue('40')
