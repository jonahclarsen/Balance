...
upgrade; newly written v5 envelopes and direct handshakes require the new app.

Direct sync splits encrypted envelopes into continuation frames of at most
16 MiB and authenticates the complete envelope before applying any operations.
It enforces a 512 MiB envelope/decoded-message limit. Large relay operations use
the existing chunked checkpoint endpoint, so no relay deployment is required.
...
content-hash verification, and encrypted transfers above the old frame limit.
