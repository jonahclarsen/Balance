...
| Direct TCP success does not acknowledge receiver application | Across four merge-delay schedules and three retries each, the receiver rejects an operation while the sender returns success. Both directions exchange real encrypted TCP frames. | The direct transport has since been removed in favor of internet-only sync; its characterization tests were retired. |
