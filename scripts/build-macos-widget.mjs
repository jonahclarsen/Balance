...
const intentsExtensionRoot = resolve(outputRoot, 'BalanceIntents.appex')
const widgetProject = resolve(root, 'src-tauri/macos/BalanceWidget.xcodeproj')
const intentsProject = resolve(root, 'src-tauri/macos/BalanceIntents.xcodeproj')
...
for (const [project, target] of [
  [widgetProject, 'BalanceWidget'],
  [intentsProject, 'BalanceIntents'],
]) {
...
execFileSync('codesign', [
  ...
  'runtime',
  '--entitlements',
  resolve(root, 'src-tauri/macos/BalanceIntents/BalanceIntents.entitlements'),
  intentsExtensionRoot,
], { stdio: 'inherit' })

console.log(`Built universal macOS extensions ${extensionRoot} and ${intentsExtensionRoot}`)
