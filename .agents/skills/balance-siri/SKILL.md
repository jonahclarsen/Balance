...
- The intent creates a one-time `balance://add?text=...&request=...` URL. The
  request identifier prevents the debug notification and normal URL activation
  from inserting the same task twice.
