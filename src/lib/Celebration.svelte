...
<style>
  ...
  .presence-words { position: absolute; z-index: 3; left: 50%; top: 63%; isolation: isolate; padding: .8em 1.08em .8em 1.34em; border-radius: 48% 52% 46% 54% / 58% 44% 56% 42%; background: linear-gradient(108deg, #3b3d70eb, #28666bed 48%, #71456fe8); box-shadow: inset 0 1px #fff5, inset 0 -8px 18px #17213b30, 0 10px 28px #2c355049; color: #fffdf7; font: 720 clamp(14px, 2.2vw, 22px)/1 system-ui; letter-spacing: .28em; text-shadow: 0 2px 8px #192345cc; white-space: nowrap; transform: translateX(-50%); animation: presence-words 4.2s ease both; }
  .presence-words::before { content: ''; position: absolute; z-index: -2; inset: -28%; border-radius: 42% 58% 55% 45% / 57% 42% 58% 43%; background: conic-gradient(from 18deg, #8de4e5cc, #d7b66ddd, #ed91cddd, #8c9cf1d9, #8de4e5cc); filter: blur(18px); opacity: .78; animation: presence-aura 2.4s ease-in-out infinite alternate; }
  .presence-words::after { content: ''; position: absolute; z-index: -1; inset: 2px; border-radius: inherit; background: radial-gradient(circle at 24% 18%, #fff4 0 9%, transparent 30%); }
  .presence-words i { color: #ffe59a; font-style: normal; text-shadow: 0 0 12px #ffd470; }
  ...
  @keyframes presence-aura { from{transform:rotate(-3deg) scale(.94);filter:blur(18px) hue-rotate(-8deg)} to{transform:rotate(4deg) scale(1.08);filter:blur(22px) hue-rotate(10deg)} }
