<script lang="ts">
  ...
  let mobileDrawerDragOffset: number | null = null
...
return rows`
  ...
  $: appShellStyle = [
    ...
    checkboxColor
      ...
      : '',
    mobileDrawerDragOffset != null ? `--mobile-drawer-offset: ${mobileDrawerDragOffset}px` : '',
  ]
    .filter(Boolean)
    .join('; ')
  $: contentShellStyle = [
  ...
  function beginMobileDrawerGesture(event: PointerEvent) {
    if (!usesMobileLayout() || event.button !== 0 || mobileDrawerGesture) return
    const startsAtEdge = event.clientX <= 28
    const startsInDrawer = mobileDrawerOpen && event.clientX <= mobileDrawerWidth()
    ...
    mobileDrawerDragOffset = mobileDrawerOpen ? 0 : -mobileDrawerWidth()
  }
  ...
  function moveMobileDrawerGesture(event: PointerEvent) {
    ...
    mobileDrawerDragOffset = mobileDrawerOpen
  ...
  function endMobileDrawerGesture(event?: PointerEvent) {
    ...
    const width = mobileDrawerWidth()
    if (gesture.axis === 'horizontal' && mobileDrawerDragOffset != null) {
      mobileDrawerOpen = mobileDrawerDragOffset > -width / 2
    }
    mobileDrawerGesture = null
    mobileDrawerDragOffset = null
  }
...
<main
  ...
  class:mobile-drawer-dragging={mobileDrawerGesture?.axis === 'horizontal'}
  style={appShellStyle}
...
>
  ...
  <aside id="primary-sidebar" class="sidebar" class:sidebar-hidden={sidebarHidden} aria-label="Primary navigation drawer">
    <div>
      <h1>Balance</h1>
  ...
  <button
    ...
    class:open={mobileDrawerOpen || mobileDrawerGesture?.axis === 'horizontal'}
    type="button"
